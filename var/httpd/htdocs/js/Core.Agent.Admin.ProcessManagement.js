...
Core.Agent.Admin.ProcessManagement = (function (TargetNS) {
    ...
    /**
     * @private
     * @name InitScopeSwitchesParent
     * @memberof Core.Agent.Admin.ProcessManagement
     * @function
     * @description
     *      switches the appropriate parent display, once at init,
     *      then anytime the scope changes
     */
    TargetNS.InitScopeSwitchesParent = function() {
        // run switch once at init
        TargetNS.ScopeSwitchesParent();

        // set up mutation observer on the scope
        var Scope = document.querySelector('#Scope');
        if( ! Scope || ! Scope.previousSibling ) { return; }
        var Target = Scope.previousSibling;

        var observer = new MutationObserver(function(mutations) { TargetNS.ScopeSwitchesParent(); });
    ...
    /**
     * @private
     * @name ScopeSwitchesParent
     * @memberof Core.Agent.Admin.ProcessManagement
     * @function
     * @description
     *      displays process item parent either as an input box,
     *      when scope is 'Process' or as a greyed out text, 
     *      when scope is global.
     *      NOTE: this code relies on the structure generated by BuildSelection
     *            notably for the location of the current value for Scope/Parent
     *            (in the previous sibling)
     */
    TargetNS.ScopeSwitchesParent = function() {
